...
case "$status" in
	start)
		if [ -n "$EXISTPID" ] ; then
			...
			exec sh "${EXIST_HOME}/bin/server.sh"
		fi
		;;
	stop)
		if [ -n "$EXISTPID" ] ; then
	...
	client)
		if [ -n "$EXISTPID" ] ; then
			...
			exec sh "${EXIST_HOME}/bin/client.sh" -l
		fi
		;;
	rclient)
		if [ -n "$EXISTPID" ] ; then
			...
			CLIENT_JAVA_OPTIONS="$JAVA_OPTIONS"
			export CLIENT_JAVA_OPTIONS
			exec sh "${EXIST_HOME}/bin/client.sh" -N -ouri=xmldb:exist//localhost:${SERVER_PORT}/xmlrpc -u admin
		else
			echo "eXist instance ${BRANCH} is NOT running!!!" 1>&2
			RETVAL=1
		fi
		;;
	*)
		echo "Usage: $0 {start|stop|status|client|kill9)" 1>&2
		;;
esac
