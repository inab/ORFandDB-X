...
<!--
	...
	Copyright (c) 1966-2006 Johns Hopkins University
...
sub referenceTagger($) {
	...
	$totag =~ s/([0-9]{6})(?:\.([0-9]{4}))?/&toRef($1,$2)/sge;
...
sub toRef($$) {
	return "<odb:link id='$_[0]' namespace='OMIM'>".(defined($_[1])?"<odb:attr name='allelicID'>$_[1]</odb:attr>":'').'<odb:text>'.$_[0].(defined($_[1])?".$_[1]":'').'</odb:text></odb:link>';
...
#		} elsif($mutdesc =~ /^EX[0-9]+(?:-[0-9]+)? ?(?:(?:INS)|(?:DEL)|(?:DUP))/) {
			...
			$mutdesc=unspecificMutation($OUTPUT,$mutdesc);
