...
*/
    ...
    EnsemblRefs er=new EnsemblRefs();
    String[] mimrefs=er.getMIMRefsForAGene("ENSG00000134982");
    String[] erefs={"135290","HAAO"};
    String[] ensrefs=er.fetchGenesByExternalRefs(erefs);
    String[] enstruerefs=er.fetchGenesByExternalMIMRefs(erefs);
    
    System.out.println("MIMs encontrados:");
    for(int imim=0;imim<mimrefs.length;imim++) {
    	System.out.println("\t"+mimrefs[imim]);
    }
    
    System.out.println("Búsqueda por sinónimos:");
    for(int isyn=0;isyn<ensrefs.length;isyn++) {
    	System.out.println("\t"+ensrefs[isyn]);
    }
    
    System.out.println("Búsqueda por sinónimos de MIM:");
    for(int isyn=0;isyn<enstruerefs.length;isyn++) {
    	System.out.println("\t"+enstruerefs[isyn]);
    }
   ...
   * @throws AdaptorException
   */
  public Test() throws AdaptorException {
	...
	// coreDriver = CoreDriverFactory.createCoreDriver("build/bioVapor.properties");
  ...
  public void fetchGenesByExternalRefs() throws AdaptorException {
    ...
    String[] xrefs = new String[] { "1553569_at", "HAAO", "135290" };
  ...
  public void fetchDeletedGeneFromArchive() 
    ...
    // fetchCurrent() is currently disabled.
